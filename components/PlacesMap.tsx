...
export default function PlacesMap({ places }: { places: Place[] }) {
  ...
  useEffect(() => {
    ...
    mapRef.current = new mapboxgl.Map({ container: ref.current, style: 'mapbox://styles/mapbox/streets-v12', center: [-1.5, 54], zoom: 5 });
  }, []);
  useEffect(() => {
    ...
    places.forEach((p) => {
      ...
      const marker = new mapboxgl.Marker(el).setLngLat([p.longitude, p.latitude]).setPopup(new mapboxgl.Popup().setHTML(`<strong>${p.name}</strong>`)).addTo(map);
