  ...
  return (
    <div>
      ...
      <ul style={{ display: 'grid', gap: 16, listStyle: 'none', padding: 0 }}>
        {events?.map((e) => (
          <li key={e.id} style={{ border: '1px solid #eee', borderRadius: 12, padding: 16 }}>
            <h3 style={{ margin: 0 }}>{e.title}</h3>
            <div style={{ fontSize: 14, color: '#666' }}>{new Date(e.starts_at).toLocaleString()}</div>
            <p>{e.description}</p>
            <details>
              <summary>Check-in</summary>
              <p style={{ fontSize: 14, color: '#666' }}>Leaders: share the 6-digit code with attendees at the meetup. Members can scan the QR or type the code to mark attendance.</p>
              <EventCheckin eventId={e.id} code={"000000"} />
            </details>
          </li>
        ))}
      </ul>
      <p style={{ fontSize: 12, color: '#666' }}>Note: The QR shows a placeholder code until you set one per event in Supabase (see README).</p>
    </div>
  );
}
