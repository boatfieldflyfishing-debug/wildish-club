...
async function togglePlace(id: string, approved: boolean) {
  'use server';
  const supabase = supabaseServer();
...
export default async function LeaderPage() {
  ...
  return (<div><h1>Leader Dashboard</h1>
    <section style={{ marginTop: 24 }}>
      ...
      <form action={createEvent} style={{ display: 'grid', gap: 8, maxWidth: 420 }}>
        ...
        <select name="branch_slug">{branches?.map((b) => <option key={b.slug} value={b.slug}>{b.name}</option>)}</select>
    ...
    <section style={{ marginTop: 24 }}>
      ...
      <ul style={{ listStyle: 'none', padding: 0, display: 'grid', gap: 8 }}>
        {events?.map((e) => (<li key={e.id} style={{ border: '1px solid #eee', borderRadius: 8, padding: 12 }}>
    ...
    <section style={{ marginTop: 24 }}>
      ...
      <ul style={{ listStyle: 'none', padding: 0, display: 'grid', gap: 8 }}>
        {places?.map((p) => (<li key={p.id} style={{ border: '1px solid #eee', borderRadius: 8, padding: 12, display: 'flex', gap: 12, alignItems: 'center' }}>
          <span style={{ width: 160 }}>{p.name}</span><code>{p.category}</code>
          <form action={async () => { await togglePlace(p.id, !p.approved); }}><button>{p.approved ? 'Unapprove' : 'Approve'}</button></form>
        </li>))}
      </ul>
    </section>
  </div>);
